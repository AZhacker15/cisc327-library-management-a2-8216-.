...
def test_valid_book_submission():
    # The first testcase.
    ...
    # The test will pass if the assertion matches the return statement
...
def test_repeated_ISBN():
    ...
    if success:
        # Returns positive when testing the first time. Which is the reason why I added this condition.
        assert success == True
        assert 'Book "The Truman Novel" has been successfully added to the catalog' in message
    else:  # The test will return False if the ISBN is repeated
        assert success == False
